def round_robin_prioridades():
    ...
    for i in range(n): #loop para coletar informações de cada processo
        ...
        priority = int(input(f"Digite a prioridade do processo {processos[i]} (quanto maior o valor, maior a prioridade): "))    #solicita a prioridade do processo em questão
    ...
    while any(r > 0 for r in restante):
        ...
        fila.sort(key=lambda i: prioridades[i], reverse=True) #ordena a fila de processos com base na prioridade, do maior para o menor valor (prioridade mais alta primeiro)
